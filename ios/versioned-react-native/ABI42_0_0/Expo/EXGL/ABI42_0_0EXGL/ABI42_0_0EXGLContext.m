...
@interface ABI42_0_0EXGLContext ()

@property (nonatomic, strong) dispatch_queue_t glQueue;
@property (nonatomic, weak) ABI42_0_0UMModuleRegistry *moduleRegistry;
@property (nonatomic, weak) ABI42_0_0EXGLObjectManager *objectManager;

@end

@implementation ABI42_0_0EXGLContext

- (instancetype)initWithDelegate:(id<ABI42_0_0EXGLContextDelegate>)delegate andModuleRegistry:(nonnull ABI42_0_0UMModuleRegistry *)moduleRegistry
{
  if (self = [super init]) {
    self.delegate = delegate;

    _moduleRegistry = moduleRegistry;
    _objectManager = (ABI42_0_0EXGLObjectManager *)[_moduleRegistry getExportedModuleOfClass:[ABI42_0_0EXGLObjectManager class]];
...
- (void)initialize:(void(^)(BOOL))callback
{
  ...
  id<ABI42_0_0UMJavaScriptContextProvider> jsContextProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI42_0_0UMJavaScriptContextProvider)];
  ...
  if (jsRuntimePtr) {
    ...
    [uiManager dispatchOnClientThread:^{
      ABI42_0_0EXGLContext *self = weakSelf;
      id<ABI42_0_0UMUIManager> uiManager = weakUIManager;
    ...
    BLOCK_SAFE_RUN(callback, NO);
    ABI42_0_0UMLogWarn(@"ABI42_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://ABI42_0_0Reactnative.dev/docs/debugging)? ABI42_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI42_0_0EXGL.");
  }
}
  ...
  [self runAsync:^{
    ...
    if (sourceFramebuffer == 0) {
      reject(
             @"E_GL_NO_FRAMEBUFFER",
             nil,
             ABI42_0_0UMErrorWithMessage(@"No framebuffer bound. Create and bind one to take a snapshot from it.")
             );
      return;
    }
    if (width <= 0 || height <= 0) {
      reject(
             ...
             nil,
             ABI42_0_0UMErrorWithMessage(@"Rect's width and height must be greater than 0. If you didn't set `rect` option, check if the viewport is set correctly.")
             );
      return;
    }
    ...
    // Begin image context
    CGFloat scale = [ABI42_0_0UMUtilities screenScale];
    ...
    if ([format isEqualToString:@"webp"]) {
      ABI42_0_0UMLogWarn(@"iOS doesn't support 'webp' representation, so 'takeSnapshot' won't work with that format. The image is going to be exported as 'png', but consider using a different code for iOS. Check this docs to learn how to do platform specific code (https://ABI42_0_0Reactnative.dev/docs/platform-specific-code)");
